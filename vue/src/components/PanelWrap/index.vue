<template>
	<div class="wrapper-content">
		<div class="panel panel-default">
			<div class="panel-title">
				...
				<ul class="panel-tools">
					<li v-if="show1" style="font-weight:normal;">
						<transition-group v-if="sortBtnType != 'relationPage'" name="el-fade-in-linear">
							<el-date-picker
							:key="1"
							@change='dataChange'
            ...
            <div class="panel-sort" v-if="sortBtnType === 'relationPage' " >
              <ul>
                <li v-for="(item, index) in relationSortItems" :class="{ 'selected': isActive === index }" v-on:click="sortChangeItem(item.conditionType,item.sortType,index)">{{item.content}}</li>
              </ul>
            </div>
